...
| `frame_ancestors` | `ATHAR_FRAME_ANCESTORS` | — | string | *(empty)* | Space-separated CSP origin allow-list for embedding the dashboard in an iframe. Empty means the dashboard sets `X-Frame-Options: DENY`; any other value switches to `Content-Security-Policy: frame-ancestors <value>`. |
